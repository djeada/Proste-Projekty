...
class TestTable(unittest.TestCase):
    def test_add_score_threes(self):
        ...
        table.add_score(score_type, dices)
        excepted_score = 9
        result = table.get_score(score_type)
        self.assertEqual(excepted_score, result)

        table = Table()
        dices = [Dice(3), Dice(3), Dice(3), Dice(1), Dice(2), Dice(5)]
        table.add_score(score_type, dices)
        result = table.get_score(score_type)
        self.assertEqual(excepted_score, result)
    ...
    def test_add_score_small_straight(self):
        table = Table()
        dices = [Dice(1), Dice(2), Dice(3), Dice(4), Dice(1)]
    ...
    def test_add_score_large_straight(self):
        table = Table()
        dices = [Dice(2), Dice(3), Dice(4), Dice(5), Dice(6)]
    ...
    def test_add_score_full_house(self):
        table = Table()
        dices = [Dice(1), Dice(1), Dice(1), Dice(2), Dice(2)]
    ...
    def test_add_score_yahtzee(self):
        table = Table()
        dices = [Dice(1), Dice(1), Dice(1), Dice(1), Dice(1)]
